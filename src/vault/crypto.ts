import type { EncryptedPayload, VaultMeta } from "../lib/types";
import { base64UrlToBytes, bytesToBase64Url, randomBytes, textBytes } from "../lib/encoding";
...
export async function importAesKey(raw: BufferSource) {
  return crypto.subtle.importKey("raw", raw, { name: "AES-GCM" }, false, ["encrypt", "decrypt"]);
}

export async function deriveWrappingKey(secret: BufferSource, purpose: string) {
...
export function writeVaultMeta(meta: VaultMeta) {
...
export function ensureVaultRuntime() {
...
export async function createVaultCredential(salt: Uint8Array) {
  ensureVaultRuntime();
...
export async function evaluateCredentialPrf(credentialId: string, salt: Uint8Array) {
  ensureVaultRuntime();
...
export async function unwrapVaultKey(meta: VaultMeta, wrappingKey: CryptoKey) {
